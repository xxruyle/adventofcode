...
def mapConvertRangeMin():  
    ...
    print(ranges) 
    min = inf 
    print(max([eval(x) for x in seeds])) 
    for i in range(max([eval(x) for x in seeds])):   
        cur = int(i) 
        prev = cur  
        for conv in reversed(seedMaps):
            cur = reverseConvertSeed(cur, conv)     


        for r in ranges: 
            if cur >= r[0] and cur <= r[1]: 
                return prev   
    


def reverseConvertSeed(num, toConvertList):  
    '''the num we want to convert and the list with the information to convert''' 
    toConvertList = toMatrix(toConvertList) 
    ranges = []    
    for m in toConvertList:  
    ...
    sourceLowerBound = cur[0][0]          
    destLowerBound = cur[1][0]  
